...
export class PlaywrightSync {
  ...
  private async captureLikesProbe(page: Page): Promise<ProbeResult> {
    ...
    const responseHandler = async (response: Awaited<ReturnType<Page["waitForResponse"]>>) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : "Unknown response read failure"
        console.error(`[sync] could not read likes response ${response.url()}: ${message}`)
      }
    }
  ...
  private async resolveLikesTimelineUrl(page: Page) {
    ...
    try {
      ...
      if (href && href.startsWith("/")) {
        return new URL(`${href.replace(/\/$/, "")}/likes`, "https://x.com").toString()
      }
    } catch {
